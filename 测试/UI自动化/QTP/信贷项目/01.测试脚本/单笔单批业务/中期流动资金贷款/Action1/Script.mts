...
For iLoop = 1 To 15 Step 1
	Call FlowControl("中期流动资金贷款",Environment("PhaseRole"))rem*不同流程的对应数字请修改
	print "需要选择的菜单："&environment("PhaseMenu")
	Login(Environment("UserID"))
	LoadAndRunAction Path,"1.1.6.1、进入菜单、选中数据、签署意见"
	If Environment("PhaseRole")="公司分部审查岗" Then
...
Next
rem*查询流水登记合同
Environment.Value("UserID")="008250"
Environment.Value("serialno")="sq2018082700000001"
...
For iRow = 1 To 30 Step 1
	...
	End If
	Call FlowControl(7,Environment("PhaseRole"))rem*不同流程的对应数字请修改
