...
Browser("合同管理").Page("合同管理").Frame("right").WebElement("登记合同").WaitProperty "visible","true",5000
...
Select Case Browser("合同管理").Page("合同管理").Frame("myiframe0").WebEdit("value:="&environment.Value("serialno")).Exist(2)
	Case true
	Browser("合同管理").Page("合同管理").Frame("myiframe0").WebEdit("value:="&environment.Value("serialno")).Click
	Browser("合同管理").Page("合同管理").Frame("right").WebElement("合同详情").Click
	Case false
...
'	.WebList("贷款投向（行内行业）").Select "钢贸"
	...
	Select Case RateType
		...
		.WebEdit("利率浮动值").Click
		.WebEdit("利率浮动值").Set "0"
		.WebList("利率调整方式").Click
		.WebList("利率调整方式").Select "次年初"
	End Select
	.WebList("还款方式").Click
