...
rem*导入数据表
	filename="信用速贷流程"rem--输入需要导入的Excel文件名称
	cFolder=Environment("TestDir")
	oParentFolder=split(cFolder,"信贷项目")(0)&"信贷项目/03.导入案例"
	DataTablePath=oParentFolder&"/"&filename&".xlsx"
	Print(DataTablePath)
	DataTable.Import  DataTablePath
	Print(DataTable.GetSheet(1).Name)
	Print(DataTable.GetSheet(2).Name)

RunAction "0、初始数据", oneIteration
'rem----------------------------------------------------------------------------------------------------------------------
'rem----------------------------------------------------------------------------------------------------------------------
't=timer
'rem'面签阶段
